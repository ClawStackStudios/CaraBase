...
- **Access Boundary:** Public key access is strictly governed by **Row-Level Security (RLS)** policies. If a table has no RLS policies, a `pk_` request will return zero rows or an explicit `403 Forbidden`.

### 2. Private LobsterService Keys (`ls-`)
...
1. It confirms the `apikey` is active. If it's a `pk_`, it arms the RLS engine.
